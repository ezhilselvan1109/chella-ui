...
export const drawerOverlayVariants = cva(
  "fixed inset-0 z-50 bg-black/60 backdrop-blur-xs transition-opacity duration-300 animate-in fade-in-0",
  {
    variants: {
      open: {
        true: "opacity-100",
...
export const drawerContentVariants = cva(
  "fixed z-50 flex flex-col bg-card text-card-foreground shadow-2xl border-border transition-transform duration-300 ease-in-out font-sans",
  {
    variants: {
      position: {
