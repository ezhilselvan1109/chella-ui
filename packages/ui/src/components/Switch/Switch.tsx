...
export const Switch = forwardRef<HTMLInputElement, SwitchProps>(
  (
    ...
    return (
      <div className={cn("inline-flex items-start gap-3 font-sans", className)}>
        <div className="relative flex items-center justify-center mt-0.5">
          {/* Native hidden accessible switch */}
          <input
            ref={ref}
            id={inputId}
            type="checkbox"
          ...
          {/* Styled visual switch track */}
          <label
            htmlFor={inputId}
            className={cn(
              switchTrackVariants({ size, state: stateVariant, disabled }),
              "peer-focus-visible:ring-2 peer-focus-visible:ring-ring peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-background"
            )}
          >
            {/* Sliding Thumb */}
            <span
              className={cn(switchThumbVariants({ size, checked: isChecked }))}
              aria-hidden="true"
            />
          </label>
        </div>
