...
export const toastVariants = cva(
  "pointer-events-auto relative flex w-full max-w-sm items-start gap-3 rounded-chellaa-lg border p-4 shadow-xl transition-all duration-300 animate-in fade-in-0 slide-in-from-top-2 font-sans select-none",
  {
    variants: {
      variant: {
...
export const toastContainerVariants = cva(
  "fixed z-50 flex flex-col gap-2.5 p-4 pointer-events-none max-h-screen overflow-hidden",
  {
    variants: {
      placement: {
