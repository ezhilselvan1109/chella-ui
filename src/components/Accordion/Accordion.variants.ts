...
export const accordionVariants = cva("w-full font-sans", {
  variants: {
    variant: {
      ...
      bordered: "divide-y divide-border rounded-chella-lg border border-border overflow-hidden",
...
export const accordionItemVariants = cva("transition-colors", {
  variants: {
    variant: {
      ...
      separated: "rounded-chella-lg border border-border bg-card px-4 shadow-xs",
    },
  },
  defaultVariants: {
    variant: "default",
...
export const accordionContentVariants = cva(
  "overflow-hidden text-xs text-muted-foreground transition-all duration-200 animate-in fade-in-0 pb-4 pt-0 leading-relaxed font-sans"
);
