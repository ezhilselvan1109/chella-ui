...
describe("Radio Component", () => {
    it("renders a native radio input", () => {
        render(_jsx(Radio, { value: "option1", "aria-label": "Option 1" }));
    ...
    it("renders with label prop and selects when label is clicked", async () => {
        ...
        render(_jsx(Radio, { value: "plan-pro", label: "Pro Tier" }));
    ...
    it("renders label passed as children", async () => {
        const user = userEvent.setup();
        render(_jsx(Radio, { value: "annual", children: "Annual Billing" }));
    ...
    it("supports uncontrolled mode with defaultChecked", () => {
        render(_jsx(Radio, { value: "starter", label: "Starter", defaultChecked: true }));
    ...
    it("supports controlled mode and fires onCheckedChange", async () => {
        ...
        const { rerender } = render(_jsx(Radio, { value: "opt", label: "Controlled Radio", checked: false, onCheckedChange: handleCheckedChange }));
        const radio = screen.getByRole("radio", { name: "Controlled Radio" });
        expect(radio).not.toBeChecked();
        await user.click(radio);
        expect(handleCheckedChange).toHaveBeenCalledWith(true);
        rerender(_jsx(Radio, { value: "opt", label: "Controlled Radio", checked: true, onCheckedChange: handleCheckedChange }));
        expect(radio).toBeChecked();
    });
    it("does not select when disabled and prevents callbacks", async () => {
        const user = userEvent.setup();
        const handleCheckedChange = vi.fn();
        render(_jsx(Radio, { value: "enterprise", label: "Enterprise", disabled: true, onCheckedChange: handleCheckedChange }));
    ...
    it("preserves name, value, and required attributes", () => {
        render(_jsx(Radio, { name: "billing", value: "monthly", label: "Monthly", required: true }));
    ...
    it("selects when pressing Space key", async () => {
        const user = userEvent.setup();
        render(_jsx(Radio, { value: "key", label: "Keyboard Option" }));
    ...
    it("associates description via aria-describedby", () => {
        render(_jsx(Radio, { value: "sms", label: "SMS Authentication", description: "Receive login codes via text message." }));
    ...
    it("associates error message and sets aria-invalid", () => {
        render(_jsx(Radio, { value: "consent", label: "Mandatory Consent", error: "Selection is required." }));
    ...
    it("supports small, medium, and large sizes", () => {
        const { container: smallContainer } = render(_jsx(Radio, { value: "s", size: "small", label: "Small" }));
        const { container: largeContainer } = render(_jsx(Radio, { value: "l", size: "large", label: "Large" }));
    ...
    it("forwards ref to underlying HTMLInputElement", () => {
        const ref = createRef();
        render(_jsx(Radio, { ref: ref, value: "ref-test", label: "Ref Test" }));
        expect(ref.current).toBeInstanceOf(HTMLInputElement);
        expect(ref.current?.tagName).toBe("INPUT");
        expect(ref.current?.type).toBe("radio");
    });
    it("merges custom className onto root container", () => {
        const { container } = render(_jsx(Radio, { value: "cls", className: "custom-radio-wrap", label: "Styled" }));
