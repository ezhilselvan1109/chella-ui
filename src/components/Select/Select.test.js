...
describe("Select component", () => {
    it("renders trigger with placeholder", () => {
        render(_jsx(Select, { options: mockOptions, placeholder: "Choose user" }));
        expect(screen.getByText("Choose user")).toBeInTheDocument();
    });
    it("opens dropdown and selects an option", async () => {
        const handleChange = vi.fn();
        const user = userEvent.setup();
        render(_jsx(Select, { options: mockOptions, onChange: handleChange, placeholder: "Choose" }));
    ...
    it("closes dropdown on Escape key", async () => {
        const user = userEvent.setup();
        render(_jsx(Select, { options: mockOptions, placeholder: "Choose" }));
        const trigger = screen.getByRole("combobox");
        await user.click(trigger);
        expect(trigger).toHaveAttribute("aria-expanded", "true");
    ...
    it("filters options when searchable is enabled", async () => {
        const user = userEvent.setup();
        render(_jsx(Select, { options: mockOptions, searchable: true, searchPlaceholder: "Type to filter..." }));
        const trigger = screen.getByRole("combobox");
        await user.click(trigger);
    ...
    it("supports multi-selection mode", async () => {
        const user = userEvent.setup();
        const handleChange = vi.fn();
        render(_jsx(Select, { options: mockOptions, multiple: true, onChange: handleChange }));
        const trigger = screen.getByRole("combobox");
        await user.click(trigger);
    ...
    it("prevents selecting disabled options", async () => {
        const handleChange = vi.fn();
        const user = userEvent.setup();
        render(_jsx(Select, { options: mockOptions, onChange: handleChange }));
        const trigger = screen.getByRole("combobox");
        await user.click(trigger);
    ...
    it("clears selection when clearable button is clicked", async () => {
        const handleChange = vi.fn();
        const user = userEvent.setup();
        render(_jsx(Select, { options: mockOptions, defaultValue: "1", clearable: true, onChange: handleChange }));
        expect(screen.getByText("Kumar")).toBeInTheDocument();
