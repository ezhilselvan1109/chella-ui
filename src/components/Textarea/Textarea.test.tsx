...
describe("Textarea component", () => {
  ...
  it("renders with label and focuses when label is clicked", async () => {
    ...
    render(<Textarea label="Bio Summary" />);
  ...
  it("renders error message and applies aria-invalid", () => {
    render(
      <Textarea
        label="Project Scope"
        error="Scope description cannot be empty."
      />
    );
  ...
  it("associates description via aria-describedby", () => {
    render(
      <Textarea
        ...
        description="Provide a high-level summary of your microservice."
      />
    );
  ...
  it("forwards ref to the underlying HTMLTextAreaElement", () => {
    ...
    render(<Textarea ref={ref} placeholder="Ref forward test" />);
    expect(ref.current).toBeInstanceOf(HTMLTextAreaElement);
    expect(ref.current?.tagName).toBe("TEXTAREA");
  });

  it("applies variant and size classes", () => {
    const { container: filledCont } = render(<Textarea variant="filled" size="small" />);
    const { container: flushedCont } = render(<Textarea variant="flushed" size="large" />);
